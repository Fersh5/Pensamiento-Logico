...
#             if values[s[char]]< values[s[char+1]]:
#                 result-=values[s[char]]
#             else:
#                 result+=values[s[char]]
...
class Solution:
    def romanToInt(self, s: str) -> int:
        ...
        for char in range(0,len(s)):
            if char+1<len(s) and values[s[char]] < values[s[char+1]]:
                result-=values[s[char]]
            else:
                result+=values[s[char]]
        return result
...
case_1=Solution()
print(case_1.romanToInt('III'))
print(case_1.romanToInt('LVIII'))
print(case_1.romanToInt('MCMXCIV'))